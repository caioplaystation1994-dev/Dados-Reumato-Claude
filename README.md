...
Aplicativos independentes, cada um em **um único arquivo HTML**, que rodam direto no navegador sem
instalação, sem servidor e sem build. Os dados ficam apenas no navegador de quem usa
(`localStorage`); nada é enviado para servidores além das consultas públicas de cotação e de
...
| `index.html` | Portal das ferramentas clínicas |
| `documentos_ambulatorio.html` | Relatório, receita e solicitação de exames |
| `laudo_imunobiologicos.html` | Laudos de autorização de imunobiológicos (DUT 65/ANS) |
| `laudo_convenio_imunobio.html` | Laudos de imunobiológicos para convênio |
...
As ferramentas clínicas são publicadas no site pelo workflow `.github/workflows/pages.yml`, que
copia os arquivos da `main` para `docs/app/` injetando `noindex` e a tela de senha.
`investimentos.html` **não** está nessa lista: hoje ele só roda a partir do arquivo local.
...
1. Abra o arquivo no navegador (ou salve na tela inicial do celular). Com a carteira ainda vazia,
   a tela inicial mostra os **primeiros passos**, que conduzem os itens 2 e 3 abaixo e somem
   sozinhos no primeiro lançamento. Abra sempre pelo mesmo endereço: o `localStorage` é separado
   por origem, então o arquivo local e o site publicado têm cada um a sua própria carteira.
...
- **Cobertura parcial**: quando a série do Banco Central começa depois da aplicação, o trecho
  ...
  que elas é cadastrado. A defasagem do **fim** da série não conta como cobertura parcial: atraso
  de divulgação é normal (o aviso só dispara acima de 2,5 meses no IPCA), e no IPCA o trecho entre
  a aplicação e o primeiro mês fechado também não conta — o índice só existe mês a mês, então
  qualquer aplicação feita no meio do mês carrega um pedaço que nenhuma série vai cobrir. Esses
  trechos continuam entrando na correção pela taxa estimada; o que muda é só o aviso.
